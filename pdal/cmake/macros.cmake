# Taken and adapted from PDAL and PDAL/python's cmake macros.cmake

function(pdal_julia_target_compile_settings target)
...
macro(PDAL_JULIA_ADD_PLUGIN _name _type _shortname)
    set(options)
    set(oneValueArgs)
    set(multiValueArgs FILES LINK_WITH INCLUDES SYSTEM_INCLUDES COMPILE_OPTIONS)
    cmake_parse_arguments(PDAL_JULIA_ADD_PLUGIN "${options}" "${oneValueArgs}"
        "${multiValueArgs}" ${ARGN})
    if(WIN32)
    ...
    add_library(${${_name}} SHARED ${PDAL_JULIA_ADD_PLUGIN_FILES})
    pdal_julia_target_compile_settings(${${_name}})
    target_include_directories(${${_name}} PRIVATE
        ${PROJECT_BINARY_DIR}/include
        ${PDAL_INCLUDE_DIR}
        ${PDAL_JULIA_ADD_PLUGIN_INCLUDES}
    )
  target_link_options(${${_name}} BEFORE PRIVATE ${PDAL_JULIA_ADD_PLUGIN_COMPILE_OPTIONS})
    target_compile_definitions(${${_name}} PRIVATE
      PDAL_JULIA_LIBRARY="${JULIA_LIBRARY}" PDAL_DLL_EXPORT)
    target_compile_definitions(${${_name}} PRIVATE PDAL_DLL_EXPORT)
    if (PDAL_JULIA_ADD_PLUGIN_SYSTEM_INCLUDES)
        target_include_directories(${${_name}} SYSTEM PRIVATE
          ${PDAL_JULIA_ADD_PLUGIN_SYSTEM_INCLUDES})
    endif()
